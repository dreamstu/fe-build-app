...
module.exports = function(grunt) {
	...
	grunt.registerMultiTask('seabuild', 'build your seajs modules.', function() {
		...
		this.files.forEach(function(file) {
			...
			try {
				if (options.uglify) {//压缩
					contents = uglify2.compressor(file.src[0], contents, _ops.uglify);
				}
			} catch (e) {
				grunt.log.error('压缩代码失败：' + e);
			}
			try {
				if (options.mislead) {//混淆
					contents = uglify2.min(contents).code;
				}
			} catch (e) {
				grunt.log.error('混淆代码失败：' + e);
			}	
